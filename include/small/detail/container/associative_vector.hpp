...
namespace small::detail {
    ...
    public /* element access */:
        ...
        at(K &&k) const {
            return element_access_implementation(std::forward<K>(k));
        }
    ...
    public /* modifiers */:
        ...
        std::pair<iterator, bool>
        insert_or_assign(K &&x, M &&obj) {
            auto [it, ok] = insert(
                value_type(std::forward<K>(x), std::forward<M>(obj)));
            if (!ok) {
        ...
        template <typename K, typename M>
        iterator
        insert_or_assign(const_iterator hint, K &&x, M &&obj) {
            auto [it, ok] = insert(
                hint,
                value_type(std::forward<K>(x)(x), std::forward<M>(obj)));
            if (!ok) {
                it->second = std::forward<M>(obj);
            }
            return std::make_pair(it, ok);
