...
namespace shift {
    ...
    template <class I> using difference_type_t = typename std::iterator_traits<I>::difference_type;

    template <class I> using iterator_category_t = typename std::iterator_traits<I>::iterator_category;

    template <class I, class Tag, class = void> constexpr bool is_category = false;
    template <class I, class Tag>
    constexpr bool is_category<I, Tag, std::enable_if_t<std::is_convertible_v<iterator_category_t<I>, Tag>>> = true;
    ...
    /// in the Ranges TS.)
    template <class I> constexpr difference_type_t<I> bounded_advance(I &i, difference_type_t<I> n, I const bound) {
    ...
    template <class ForwardIt> ForwardIt shift_left(ForwardIt first, ForwardIt last, difference_type_t<ForwardIt> n) {
        if (n <= 0) {
            return last;
        }
    ...
    template <class ForwardIt> ForwardIt shift_right(ForwardIt first, ForwardIt last, difference_type_t<ForwardIt> n) {
        if (n <= 0) {
            return first;
        }
