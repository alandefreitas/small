...
namespace small {
    namespace detail {
        ...
        class associative_vector {
                ...
                if (emplace_pos == end()) {
                    ...
                    if (IsMulti || !same_key) {
                        typename vector_type::iterator emplaced_it =
                            data_.emplace(maybe_base(emplace_pos), std::move(tmp));
            ...
            template <class P>
            std::enable_if_t<std::is_constructible_v<value_type, P &&>, iterator> insert(const_iterator position,
                                                                                         P &&p) {
            ...
            iterator upper_bound(const key_type &k) {
                return const_key_iterator(std::upper_bound(data_.begin(), data_.end(), k,
                                                           [this](const typename vector_type::value_type &p,
          ...
          private:
            ...
            constexpr mapped_type &element_access_implementation(const K &k) {
                if constexpr (IsMap) {
                    iterator it = find(k);
                    if (it == end()) {
            ...
            /// \brief Logic to access a mapped_type
            template <bool create_if_not_found, class K>
            constexpr mapped_type &element_access_implementation(K &&k) const {
