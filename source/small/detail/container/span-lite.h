...
namespace nonstd {
    namespace span_lite {
        ...
        template <class T, extent_t Extent = dynamic_extent> class span;
        ...
        namespace std17 {
            ...
            template <typename C> inline span_constexpr auto size(C const &cont) -> decltype(cont.size()) {
                return cont.size();
            }
            ...
            template <typename C> inline span_constexpr auto data(C &cont) -> decltype(cont.data()) {
                return cont.data();
            }

            template <typename C> inline span_constexpr auto data(C const &cont) -> decltype(cont.data()) {
                return cont.data();
            }
        ...
        namespace std20 {
            ...
            template <class T> using iter_reference_t = decltype(*std::declval<T &>());
#endif
        ...
        namespace detail {
            ...
            template <typename T> bool is_positive(T x) { return std11::is_signed<T>::value ? x >= 0 : true; }
            ...
            template <class, class = void> struct has_size_and_data : std::false_type {};

            template <class C>
            struct has_size_and_data<
                C, std17::void_t<decltype(std17::size(std::declval<C>())), decltype(std17::data(std::declval<C>()))>>
                : std::true_type {};

            template <class, class, class = void> struct is_compatible_element : std::false_type {};
...
#pragma GCC diagnostic ignored "-Wlong-long"
            ...
            inline void report_contract_violation(char const *msg) { throw contract_violation(msg); }
...
#define span_sizeof(T) static_cast<extent_t>(sizeof(T))
        ...
        template <class T> inline span_constexpr size_t to_size(T size) { return static_cast<size_t>(size); }
          ...
          public:
...
#if span_HAVE(CONSTRAINED_SPAN_CONTAINER_CTOR)
            template <
                class Container span_REQUIRES_T((detail::is_compatible_container<Container, element_type>::value))>
            ...
            template <class OtherElementType,
                      extent_type OtherExtent
                          span_REQUIRES_T(((Extent == dynamic_extent || Extent == OtherExtent) &&
            ...
            template <extent_type Count> span_constexpr_exp span<element_type, Count> first() const {
                span_EXPECTS(detail::is_positive(Count) && Count <= size());

                return span<element_type, Count>(data(), Count);
            }

            template <extent_type Count> span_constexpr_exp span<element_type, Count> last() const {
                span_EXPECTS(detail::is_positive(Count) && Count <= size());
            ...
            span_constexpr std::ptrdiff_t ssize() const span_noexcept { return static_cast<std::ptrdiff_t>(size_); }

            span_constexpr size_type size_bytes() const span_noexcept { return size() * to_size(sizeof(element_type)); }

            span_nodiscard span_constexpr bool empty() const span_noexcept { return size() == 0; }
            ...
            span_constexpr reverse_iterator rbegin() const span_noexcept { return reverse_iterator(end()); }

            span_constexpr reverse_iterator rend() const span_noexcept { return reverse_iterator(begin()); }
...
#if span_HAVE(ITERATOR_CTOR)
            static inline span_constexpr pointer to_address(std::nullptr_t) span_noexcept { return nullptr; }

            template <typename U> static inline span_constexpr U *to_address(U *p) span_noexcept { return p; }
        ...
        template <class T, size_t N> span(T (&)[N]) -> span<T, static_cast<extent_t>(N)>;

        template <class T, size_t N> span(std::array<T, N> &) -> span<T, static_cast<extent_t>(N)>;

        template <class T, size_t N> span(std::array<T, N> const &) -> span<const T, static_cast<extent_t>(N)>;
...
#else
            return span<std17::byte, BytesExtent<T, Extent>::value>(reinterpret_cast<std17::byte *>(spn.data()),
...
namespace nonstd {
    namespace span_lite {
        ...
        template <class T> inline span_constexpr span<T> make_span(T *ptr, size_t count) span_noexcept {
            return span<T>(ptr, count);
        }

        template <class T> inline span_constexpr span<T> make_span(T *first, T *last) span_noexcept {
        ...
        template <class T> inline span_constexpr span<T> make_span(span<T> spn) span_noexcept { return spn; }
        ...
        template <extent_t Count, class T, extent_t Extent> span_constexpr span<T, Count> first(span<T, Extent> spn) {
            return spn.template first<Count>();
        }

        template <class T, extent_t Extent> span_constexpr span<T> first(span<T, Extent> spn, size_t count) {
            return spn.first(count);
        }

        template <extent_t Count, class T, extent_t Extent> span_constexpr span<T, Count> last(span<T, Extent> spn) {
            return spn.template last<Count>();
        }

        template <class T, extent_t Extent> span_constexpr span<T> last(span<T, Extent> spn, size_t count) {
        ...
        template <class T> span_constexpr auto first(T &t, size_t count) -> decltype(make_span(t).first(count)) {
...
namespace std {
    ...
    template <std::size_t I, typename T> class tuple_element; /* undefined */
} // namespace std
...
namespace std {
    ...
    template <typename ElementType> class tuple_size<nonstd::span<ElementType, nonstd::dynamic_extent>>;
