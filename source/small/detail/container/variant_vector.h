...
namespace small {
    namespace detail {
        ...
        class small_vector {
          private:
            ...
            constexpr const_reference at(size_type n) const {
                if (n >= size()) {
                    this->throw_out_of_range("at({0}) const: cannot access element {0} in array of size {1}", n,
                                             size());
                }
                return begin()[n];
            }
            ...
            constexpr iterator erase(const_iterator first, const_iterator last) {
                ...
                if (std::holds_alternative<small_vector_type>(data_)) {
                    auto &data_as_array = std::get<small_vector_type>(data_);
                    return iterator(
                        &*data_as_array.erase(data_as_array.begin() + p_first, data_as_array.begin() + p_last));
                } else {
                    auto &data_as_vector = std::get<large_vector_type>(data_);
                    return iterator(
        ...
        constexpr bool operator==(const small_vector<T, N> &x, const small_vector<T, N> &y) {
        ...
        /// \brief swap the contents of two small arrays
        template <class T, size_t N>
        void swap(small_vector<T, N> &x, small_vector<T, N> &y) noexcept(noexcept(x.swap(y))) {
            x.swap(y);
        }
