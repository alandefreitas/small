...
namespace small {
    namespace detail {
        ...
        class scope_guard_impl_base {
          ...
          protected:
            /// Create the guard
            explicit scope_guard_impl_base(bool dismissed = false) noexcept : dismissed_(dismissed) {}

            /// Terminate if we have an exception
            inline static void terminate() noexcept {
                // Ensure the availability of std::cerr
                std::ios_base::Init ioInit;
                std::cerr << "This program will now terminate because a scope_guard callback "
                             "threw an \nexception.\n";
                std::rethrow_exception(std::current_exception());
            }

            static scope_guard_impl_base make_empty_scope_guard() noexcept { return scope_guard_impl_base{}; }

            template <typename T> static const T &as_const(const T &t) noexcept { return t; }

            bool dismissed_;
        };

        /// \brief A scope guard that calls a function when being destructed unless told otherwise
        template <typename FunctionType, bool InvokeNoexcept> class scope_guard_impl : public scope_guard_impl_base {
          public:
            explicit scope_guard_impl(FunctionType &fn) noexcept(
                std::is_nothrow_copy_constructible<FunctionType>::value)
                : scope_guard_impl(as_const(fn),
                                   makeFailsafe(std::is_nothrow_copy_constructible<FunctionType>{}, &fn)) {}

            explicit scope_guard_impl(const FunctionType &fn) noexcept(
                std::is_nothrow_copy_constructible<FunctionType>::value)
                : scope_guard_impl(fn, makeFailsafe(std::is_nothrow_copy_constructible<FunctionType>{}, &fn)) {}
            ...
            explicit scope_guard_impl(FunctionType &&fn, scope_guard_dismissed) noexcept(
                std::is_nothrow_move_constructible<FunctionType>::value)
                // No need for failsafe in this case, as the guard is dismissed.
                : scope_guard_impl_base{true}, function_(std::forward<FunctionType>(fn)) {}

            scope_guard_impl(scope_guard_impl &&other) noexcept(std::is_nothrow_move_constructible<FunctionType>::value)
        ...
        using scope_guard_impl_decay = scope_guard_impl<typename std::decay<F>::type, InvokeNoExcept>;

    } // namespace detail

    /// \brief The default scope guard alias for a function
    template <class F> using scope_guard = detail::scope_guard_impl_decay<F, true>;

    /// \brief Make a scope guard with a function
    template <typename F>
    [[nodiscard]] scope_guard<F> make_guard(F &&f) noexcept(noexcept(scope_guard<F>(static_cast<F &&>(f)))) {
