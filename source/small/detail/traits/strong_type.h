...
namespace strong {
    ...
    template <typename M, typename T> using modifier = typename M::template modifier<T>;
    ...
    template <typename T, typename Tag, typename... M> class type : public modifier<M, type<T, Tag, M...>>... {
      public:
        template <typename TT = T, typename = std::enable_if_t<std::is_trivially_constructible<TT>{}>>
        explicit type(uninitialized_t) noexcept {}
        template <typename type_ = type, bool = impl::supports_default_construction(static_cast<type_ *>(nullptr))>
        constexpr type() noexcept(noexcept(T{})) : val{} {}

        template <typename U, typename = impl::WhenConstructible<T, std::initializer_list<U>>>
        ...
        constexpr explicit type(U &&...u) noexcept(std::is_nothrow_constructible<T, U...>::value)
            : val(std::forward<U>(u)...) {}

        friend STRONG_CONSTEXPR void swap(type &a, type &b) noexcept(
            std::is_nothrow_move_constructible<type>::value &&std::is_nothrow_move_assignable<type>::value) {
            using std::swap;
            swap(a.val, b.val);
        }
    ...
    namespace impl {
        ...
        template <typename T, typename Tag, typename... Ms> constexpr T underlying_type(strong::type<T, Tag, Ms...> *);
    ...
    namespace impl {
        template <typename T> using WhenStrongType = std::enable_if_t<is_strong_type<std::decay_t<T>>::value>;
        template <typename T> using WhenNotStrongType = std::enable_if_t<!is_strong_type<std::decay_t<T>>::value>;
    } // namespace impl
    ...
    namespace impl {
        ...
        STRONG_NODISCARD constexpr auto access(T &&t) noexcept -> decltype(value_of(std::forward<T>(t))) {
    ...
    template <typename T, typename Tag, typename... M> class equality::modifier<::strong::type<T, Tag, M...>> {
        using type = ::strong::type<T, Tag, M...>;

      public:
        STRONG_NODISCARD
        friend STRONG_CONSTEXPR auto
        operator==(const type &lh,
                   const type &rh) noexcept(noexcept(std::declval<const T &>() == std::declval<const T &>()))
    ...
    namespace impl {
        template <typename T, typename Other> class typed_equality {
          ...
          public:
            ...
            friend STRONG_CONSTEXPR auto
            operator==(const T &lh,
                       const Other &rh) noexcept(noexcept(std::declval<const TT &>() == std::declval<const OT &>()))
                -> decltype(std::declval<const TT &>() == std::declval<const OT &>()) {
                return value_of(lh) == impl::access(rh);
            }
            STRONG_NODISCARD
            friend STRONG_CONSTEXPR auto
            operator==(const Other &lh,
                       const T &rh) noexcept(noexcept(std::declval<const OT &>() == std::declval<const TT &>()))
    ...
    class semiregular::modifier<::strong::type<T, Tag, M...>> : public default_constructible::modifier<T>,
                                                                private impl::require_semiregular<T> {};

    struct regular {
        template <typename T> class modifier : public semiregular::modifier<T>, public equality::modifier<T> {};
    };
    ...
    template <typename T, typename Tag, typename... M> class ordered::modifier<::strong::type<T, Tag, M...>> {
        using type = ::strong::type<T, Tag, M...>;
    ...
    struct iostreamable {
        template <typename T> class modifier : public ostreamable::modifier<T>, public istreamable::modifier<T> {};
    };

    struct incrementable {
        template <typename T> class modifier {
          public:
            friend STRONG_CONSTEXPR T &operator++(T &t) noexcept(noexcept(++std::declval<T &>().value_of())) {
                ++value_of(t);
                return t;
            }

            friend STRONG_CONSTEXPR T operator++(T &t, int) {
                auto copy = t;
                ++t;
                return copy;
            }
    ...
    struct bicrementable {
        template <typename T> class modifier : public incrementable::modifier<T>, public decrementable::modifier<T> {};
    };
    ...
    template <typename T, typename Tag, typename... M>
    class difference::modifier<::strong::type<T, Tag, M...>> : public ordered::modifier<::strong::type<T, Tag, M...>> {
        using type = ::strong::type<T, Tag, M...>;

      public:
        friend STRONG_CONSTEXPR type &operator+=(type &lh,
                                                 const type &rh) noexcept(noexcept(value_of(lh) += value_of(rh))) {
        ...
        friend STRONG_CONSTEXPR type &operator*=(type &lh, const T &rh) noexcept(noexcept(value_of(lh) *= rh)) {
            value_of(lh) *= rh;
            return lh;
        }
        ...
        friend STRONG_CONSTEXPR type operator+(type lh, const type &rh) {
            lh += rh;
            return lh;
        }
        ...
        friend STRONG_CONSTEXPR type operator*(type lh, const T &rh) {
            lh *= rh;
            return lh;
        }

        friend STRONG_CONSTEXPR type operator*(const T &lh, type rh) {
            rh *= lh;
            return rh;
        }
        ...
        friend STRONG_CONSTEXPR T operator/(const type &lh, const type &rh) { return value_of(lh) / value_of(rh); }
    };

    template <typename D> struct affine_point { template <typename T> class modifier; };
    ...
    class affine_point<D>::modifier<::strong::type<T, Tag, M...>> {
      ...
      public:
        STRONG_NODISCARD
        friend STRONG_CONSTEXPR D operator-(const type &lh, const type &rh) { return D(value_of(lh) - value_of(rh)); }

        friend STRONG_CONSTEXPR type &operator+=(type &lh,
                                                 const D &d) noexcept(noexcept(value_of(lh) += impl::access(d))) {
        ...
        STRONG_NODISCARD
        friend STRONG_CONSTEXPR type operator+(type lh, const D &d) { return lh += d; }

        STRONG_NODISCARD
        friend STRONG_CONSTEXPR type operator+(const D &d, type rh) { return rh += d; }
    ...
    template <typename T, typename Tag, typename... M> class pointer::modifier<::strong::type<T, Tag, M...>> {
        using type = strong::type<T, Tag, M...>;

      public:
        template <typename TT = T>
        STRONG_NODISCARD friend STRONG_CONSTEXPR auto
        operator==(const type &t, std::nullptr_t) noexcept(noexcept(std::declval<const TT &>() == nullptr))
        ...
        template <typename TT = T>
        STRONG_NODISCARD friend STRONG_CONSTEXPR auto
        operator==(std::nullptr_t, const type &t) noexcept(noexcept(nullptr == std::declval<const TT &>()))
    ...
    struct arithmetic {
        template <typename T> class modifier {
          public:
            STRONG_NODISCARD
            friend STRONG_CONSTEXPR T operator-(const T &lh) { return T{-value_of(lh)}; }

            friend STRONG_CONSTEXPR T &operator+=(T &lh, const T &rh) noexcept(noexcept(value_of(lh) += value_of(rh))) {
                value_of(lh) += value_of(rh);
                return lh;
            }
            ...
            STRONG_NODISCARD
            friend STRONG_CONSTEXPR T operator+(T lh, const T &rh) {
                lh += rh;
                return lh;
            }
    ...
    struct bitarithmetic {
        template <typename T> class modifier {
          public:
            ...
            template <typename C>
            friend STRONG_CONSTEXPR T &operator<<=(T &lh, C c) noexcept(noexcept(value_of(lh) <<= c)) {
                value_of(lh) <<= c;
                return lh;
            }
            ...
            STRONG_NODISCARD
            friend STRONG_CONSTEXPR T operator~(const T &lh) {
                auto v = value_of(lh);
                v = ~v;
                return T(v);
            }
            ...
            template <typename C> STRONG_NODISCARD friend STRONG_CONSTEXPR T operator<<(T lh, C c) {
                lh <<= c;
                return lh;
            }
    ...
    template <typename I = void> struct indexed { template <typename T> class modifier; };
    ...
    template <> struct indexed<void> {
        ...
        template <typename T, typename Tag, typename... Ms> class modifier<type<T, Tag, Ms...>> {
          ...
          public:
            template <typename I>
            STRONG_NODISCARD auto
            operator[](const I &i) const &noexcept(noexcept(std::declval<cref>()[impl::access(i)]))
            ...
            template <typename I>
            STRONG_NODISCARD auto operator[](const I &i) &noexcept(noexcept(std::declval<ref>()[impl::access(i)]))
            ...
            template <typename I>
            STRONG_NODISCARD auto operator[](const I &i) &&noexcept(noexcept(std::declval<rref>()[impl::access(i)]))
            ...
            STRONG_NODISCARD auto at(const I &i) const & -> decltype(std::declval<C>().at(impl::access(i))) {
                auto &self = static_cast<const type &>(*this);
                return value_of(self).at(impl::access(i));
            }

            template <typename I, typename R = ref>
            STRONG_NODISCARD auto at(const I &i) & -> decltype(std::declval<R>().at(impl::access(i))) {
                auto &self = static_cast<type &>(*this);
                return value_of(self).at(impl::access(i));
            }
    ...
    class indexed<I>::modifier<type<T, Tag, M...>> {
      ...
      public:
        STRONG_NODISCARD
        auto operator[](const I &i) const &noexcept(noexcept(std::declval<const T &>()[impl::access(i)]))
        ...
        STRONG_NODISCARD
        auto operator[](const I &i) &noexcept(noexcept(std::declval<T &>()[impl::access(i)]))
        ...
        STRONG_NODISCARD
        auto at(const I &i) const & -> decltype(std::declval<const T &>().at(impl::access(i))) {
            auto &self = static_cast<const type &>(*this);
            return value_of(self).at(impl::access(i));
        }

        STRONG_NODISCARD
        auto at(const I &i) & -> decltype(std::declval<T &>().at(impl::access(i))) {
            auto &self = static_cast<type &>(*this);
            return value_of(self).at(impl::access(i));
        }
    ...
    class iterator {
      public:
        ...
        class modifier : public pointer::modifier<I>, public equality::modifier<I>, public incrementable::modifier<I> {
          public:
        ...
        template <typename I>
        class modifier<I, std::bidirectional_iterator_tag> : public modifier<I, std::forward_iterator_tag>,
        ...
        class modifier<I, std::random_access_iterator_tag>
            : public modifier<I, std::bidirectional_iterator_tag>,
              public affine_point<
                  typename std::iterator_traits<underlying_type_t<I>>::difference_type>::template modifier<I>,
    ...
    template <typename T, typename Tag, typename... M> class range::modifier<type<T, Tag, M...>> {
      ...
      public:
        ...
        iterator begin() noexcept(noexcept(std::declval<T &>().begin())) {
            auto &self = static_cast<type &>(*this);
            return iterator{value_of(self).begin()};
        }

        iterator end() noexcept(noexcept(std::declval<T &>().end())) {
            auto &self = static_cast<type &>(*this);
            return iterator{value_of(self).end()};
        }

        const_iterator cbegin() const noexcept(noexcept(std::declval<const T &>().begin())) {
            auto &self = static_cast<const type &>(*this);
            return const_iterator{value_of(self).begin()};
        }

        const_iterator cend() const noexcept(noexcept(std::declval<const T &>().end())) {
            auto &self = static_cast<const type &>(*this);
            return const_iterator{value_of(self).end()};
        }

        const_iterator begin() const noexcept(noexcept(std::declval<const T &>().begin())) {
            auto &self = static_cast<const type &>(*this);
            return const_iterator{value_of(self).begin()};
        }

        const_iterator end() const noexcept(noexcept(std::declval<const T &>().end())) {
            auto &self = static_cast<const type &>(*this);
            return const_iterator{value_of(self).end()};
        }
    };
