...
#ifndef SMALL_STRING_H8_STRING_H
#define SMALL_STRING_H8_STRING_H

#include <sstream>

#include "small/detail/traits/strong_type.h"

#include "detail/algorithm/utf.h"
#include "detail/container/lookup_table_view.h"
#include "detail/iterator/codepoint_iterator.h"
#include "detail/traits/extract_value_type.h"
...
namespace small {
    ...
    /// - allow the same small/inline/cache optimization for strings (like we do for vectors) on all platforms
    ...
    /// - supporting utf8 by default (since it's a new container anyway) while still being compatible
    ///   with the string API (iterators to bytes rather than code points)
    ///     - supporting utf8 is also what makes it small because it spends up to 4x less memory compared to some
    ///       common alternatives to represent general strings
    /// - providing some basic operations to convert to wide chars and manipulate utf8 by default when these are needed
    ///     - attempt to stream to cout/cerr/... buffers without having to worry too much about converting to wide chars
    ///     - create a UTF8 string from literal strings without worrying too much about literal char type
    ///     - avoid using libraries such as boost::nowide unless we really have to
    ///
    /// The bits of this implementation that handle utf8 are adapted from the tiny-utf8 string functions.
    /// If you are used to tiny-utf8, the main differences from a user perspective are:
    /// - the custom max expected size
    /// - changes in the API to make it compatible with std::string so it can be a drop-in replacement in most cases:
    ///     - bytes from operator[] rather than code points by default (operator() is reserved for code-points)
    ///     - byte iterators from begin()/end()/insert/... rather than code points by default
    ///         - functions whose return types handle code points are prefixed with cp_* (cp_begin()/cp_end()/...)
    ///     - we attempt to provide the complete std::string interface (reserve/max_size/variations of insert/...)
    /// - Assumes C++17
    /// From an implementation perspective:
    /// - the optimizations are more like our small vectors
    /// - better growth factors
    /// - adjustments to reuse the lookup table more efficiently with these growth factors
    /// - using our own inline vectors for storage to reduce code bloat, simplify the API, make it safer, inherit
    ///   optimizations and remove some unneeded fields in the tiny-utf8 data structure for inline storage
    ///
    /// Although we attempt to minimize unneeded differences, note that this string is not an exact
    /// replacement for std::string in this case.
    /// In particular, this is not a drop-in replacement if you depend on std::string to represent
    /// bytes with your string.
    /// The main reason is that small::string will convert to wchar (usually on windows) if it has to.
    /// Although this is relatively conservative (only to std::cout), this might break things.
    /// In other words, this small string is intended to represent words/sentences/etc... rather than bytes.
    /// For representing bytes you can always use std::vector<uint8_t> or std::span instead.
    ///
    /// Reference:
    /// - https://github.com/DuffsDevice/tiny-utf8
    ///   - already has SSO, so we only we only need to allow setting the size
    ///   - in our applications, we need to make it compatible with the string API though
    ///      - So the iterators still point to bytes rather than code points
    ///      - cp_begin()/cp_end() point to chars / wcp_begin()/wchar() point to code points
    ///   - the release function allows us to move to a string
    ///
    /// \see UTF8 string implementation: https://github.com/DuffsDevice/tiny-utf8
    /// \see UTF8 functions: https://github.com/sheredom/utf8.h
    /// \note fmt::print from the fmt library supports printing as utf8 on windows so we can reuse that
    namespace detail {
        constexpr size_t default_codepoint_hint_step = 10;
    }

    template <typename CharT = char, size_t N = default_inline_storage_v<CharT>, class Traits = std::char_traits<CharT>,
              typename WCharT = char32_t, class Allocator = std::allocator<CharT>,
              size_t CP_HINT_STEP = detail::default_codepoint_hint_step, class SizeType = size_t>
    class basic_string {
      public:
        ...
        /// \section public types relative to the utf8 functionality / code points

        /// \section Type we use for represent larger codepoints
        typedef WCharT wide_value_type;

        /// \brief Data type capable of holding the number of code units in a codepoint
      ...
      public:
        /// \brief A strong type to allow overloads looking for codepoint indexes rather than byte indexes
        using codepoint_size_type =
            strong::type<size_type, codepoint_size_type_tag, strong::implicitly_convertible_to<size_type>>;

        using codepoint_index = codepoint_size_type;

      private:
        /// \brief The std::string functions that work with string views use this predicate for overload resolution
        /// This predicate means something can become a string view but not a c-string pointer, for which the API
        /// has other overloads.
      ...
      public:
        ...
        constexpr basic_string() noexcept(noexcept(std::is_nothrow_default_constructible<allocator_type>()))
        ...
        /// This is a somewhat special case because we already know how many bytes we need for
        /// the whole string. We are not usually that lucky.
        template <typename InputChar>
        constexpr basic_string(size_type count, InputChar cp, const allocator_type &alloc = allocator_type())
            : buffer_(alloc) {
            ...
            // Initialize buffer
            const size_type n_multibyte_codepoints = codeunits_per_codepoint != 1 ? count : 0;
            const size_type lookup_buffer_size =
                has_lookup_table ? lookup_table_type::size_for(size_.codeunit_size(), n_multibyte_codepoints) : 0;
        ...
        /// \brief Constructs the string with the contents of the range [first, last) of wide chars
        template <class InputIt>
        constexpr basic_string(InputIt first, InputIt last, const allocator_type &alloc = allocator_type())
            : buffer_(alloc) {
            // Handle the most trivial case first
            if (first == last) {
                buffer_.resize(has_lookup_table ? 2 : 1);
            ...
            // Initialize buffer
            const size_type lookup_buffer_size =
                has_lookup_table ? lookup_table_type::size_for(size_.codeunit_size(), output_multibyte_codepoints) : 0;
            buffer_.resize(size_.codeunit_size() + 1 + lookup_buffer_size);
            buffer_.resize(buffer_.capacity());
        ...
        /// \brief Constructs the string with a substring [pos, end) of other string
        constexpr basic_string(const basic_string &other, size_type pos, const allocator_type &alloc = allocator_type())
        ...
        constexpr basic_string(const basic_string &other, size_type pos, size_type count,
            ...
            : basic_string(other.begin() + pos, count != npos ? (other.begin() + pos + count) : other.end(), alloc) {}
        ...
        /// \brief Move constructor. Constructs the string with the contents of other using move semantics. other is
        ...
        constexpr basic_string(basic_string &&other, const allocator_type &alloc)
            : buffer_(std::move(other.buffer_), alloc), size_(std::move(other.size_)) {
            other.clear();
        }

        /// \brief Constructs the string with the contents of the initializer list ilist
        template <typename CharType>
        constexpr basic_string(std::initializer_list<CharType> ilist, const allocator_type &alloc = allocator_type())
            : basic_string(ilist.begin(), ilist.end(), alloc) {}

        /// \brief Construct from string view
        template <typename CharType, class TraitsType>
        explicit constexpr basic_string(const std::basic_string_view<CharType, TraitsType> &sv,
                                        const allocator_type &alloc = allocator_type())
            : basic_string(sv.begin(), sv.end(), alloc) {}

        /// \brief Construct from something convertible to a string view
        /// Implicitly converts t to a string view sv as if by std::basic_string_view<value_type, traits_type> sv =
        /// t;, then initializes the string with the contents of sv, as if by basic_string(sv.data(), sv.size(),
        /// alloc). This overload participates in overload resolution only if std::is_convertible_v<const T&,
        /// std::basic_string_view<value_type, traits_type>> is true and std::is_convertible_v<const T&, const
        /// value_type*> is false.
        template <class T, std::enable_if_t<is_api_string_view_v<T>, int> = 0>
        explicit constexpr basic_string(const T &t, const allocator_type &alloc = allocator_type())
            : basic_string(std::basic_string_view<typename T::value_type, std::char_traits<typename T::value_type>>(t),
                           alloc) {}

        /// \brief Implicitly converts t to a string view sv as if by std::basic_string_view<value_type, traits_type>
        /// sv = t;, then initializes the string with the subrange [pos, pos + n) of sv as if by
        /// basic_string(sv.substr(pos, n), alloc).
        template <class T, std::enable_if_t<is_api_string_view_v<T>, int> = 0>
        explicit constexpr basic_string(const T &t, size_type pos, size_type n,
                                        const allocator_type &alloc = allocator_type())
            : basic_string(
                  std::basic_string_view<typename T::value_type, std::char_traits<typename T::value_type>>(t).substr(
                      pos, n),
                  alloc) {}
        ...
        template <typename Char> constexpr basic_string &operator=(const Char *s) {
        ...
        /// \brief Replaces the contents with those of the initializer list ilist as if by assign(ilist.begin(),
        /// ilist.size())
        template <typename Char> constexpr basic_string &operator=(std::initializer_list<Char> ilist) {
            assign(ilist.begin(), ilist.size());
            return *this;
        }

        /// \brief Implicitly converts t to a string view sv and assign
        /// As if by std::basic_string_view<CharT, Traits> sv = t;, then replaces the contents
        ...
        template <typename Char> constexpr basic_string &assign(size_type count, Char ch) {
        ...
        constexpr basic_string &assign(const basic_string &str, size_type pos, size_type count = basic_string::npos) {
        ...
        template <typename Char> constexpr basic_string &assign(const Char *s, size_type count) {
        ...
        /// \brief Replaces the contents with those of null-terminated character string pointed to by s.
        template <typename Char> constexpr basic_string &assign(const Char *s) {
        ...
        template <typename Char> constexpr basic_string &assign(std::initializer_list<Char> ilist) {
        ...
        /// \brief Implicitly converts t to a string view sv as if by std::basic_string_view<CharT, Traits> sv = t;,
        /// then replaces the contents with the characters from the subview [pos, pos+count) of sv
        template <class T, std::enable_if_t<is_api_string_view_v<T>, int> = 0>
        constexpr basic_string &assign(const T &t, size_type pos, size_type count = basic_string::npos) {
            *this = basic_string(t, pos, count, get_allocator());
            return *this;
        }

        /// \brief Returns the allocator associated with the string
        [[nodiscard]] constexpr allocator_type get_allocator() const noexcept { return buffer_.get_allocator(); }
      ...
      public:
        ...
        /// \brief Returns a reference to the character at specified location pos
        /// Bounds checking is performed, exception of type std::out_of_range will be thrown on invalid access
        [[nodiscard]] constexpr const_reference at(size_type byte_index) const {
        ...
        /// \brief Returns a wide char with the i-th code point in the string
        /// Bounds checking is performed, exception of type std::out_of_range will be thrown on invalid access
        [[nodiscard]] constexpr const_codepoint_reference at(codepoint_index index) const {
        ...
        constexpr reference operator[](size_type byte_index) {
            assert(byte_index <= size() && "string::operator[]: index out of bounds");
        ...
        /// No bounds checking is performed. If pos > size(), the behavior is undefined
        /// If pos == size(), a reference to the character with value CharT() (the null character) is returned.
        constexpr const_reference operator[](size_type byte_index) const {
            assert(byte_index <= size() && "string::operator[]: index out of bounds");
            return *(data() + byte_index);
        }
        ...
        /// \brief Returns a wide char with the i-th code point in the string
        /// No bounds checking is performed. If pos > size(), the behavior is undefined
        constexpr codepoint_reference operator[](codepoint_index index) {
        ...
        /// \brief Returns a wide char with the i-th code point in the string
        /// No bounds checking is performed. If pos > size(), the behavior is undefined
        constexpr const_codepoint_reference operator[](codepoint_index index) const {
        ...
        /// No bounds checking is performed. If pos > size(), the behavior is undefined
        constexpr wide_value_type operator()(codepoint_index index) const {
            assert(index <= size_codepoints() && "string::operator(): codepoint index out of bounds");
            const_codepoint_iterator it = cbegin_codepoint() + index;
            return it.wide_value();
        }

        /// \brief Returns a byte index of a codepoint
        [[nodiscard]] constexpr wide_value_type byte_index(codepoint_index index) const {
            assert(index <= size_codepoints() && "string::operator(): codepoint index out of bounds");
        ...
        /// The behavior is undefined if empty() == true
        constexpr codepoint_reference front_codepoint() { return operator[](codepoint_index(0)); }

        /// \brief Returns value of the first codepoint in the string
        /// The behavior is undefined if empty() == true
        [[nodiscard]] constexpr const_codepoint_reference front_codepoint() const {
        ...
        /// The behavior is undefined if empty() == true
        constexpr codepoint_reference back_codepoint() { return operator[](codepoint_index(size_codepoints() - 1)); }

        /// \brief Returns reference to the last code point in the string
        /// The behavior is undefined if empty() == true
        [[nodiscard]] constexpr const_codepoint_reference back_codepoint() const {
        ...
        constexpr pointer data() noexcept { return this->buffer_.data(); }
        ...
        /// The returned array is null-terminated, that is, data() and c_str() perform the same function
        /// If empty() returns true, the pointer points to a single null character.
        [[nodiscard]] constexpr const_pointer data() const noexcept { return this->buffer_.data(); }
        ...
        /// stored in the string
        [[nodiscard]] constexpr std::basic_string<CharT> cpp_str() const {
      ...
      public:
        ...
        /// \brief Returns an iterator to the first character of the string
        [[nodiscard]] constexpr const_iterator begin() const noexcept { return buffer_.cbegin(); }

        /// \brief Returns an iterator to the first character of the string
        [[nodiscard]] constexpr const_iterator cbegin() const noexcept { return buffer_.cbegin(); }
        ...
        constexpr iterator end() noexcept { return buffer_.begin() + size(); }

        /// \brief Returns an iterator to the character following the last character of the string
        /// This character acts as a placeholder, attempting to access it results in undefined behavior
        [[nodiscard]] constexpr const_iterator end() const noexcept { return buffer_.cbegin() + size(); }

        /// \brief Returns an iterator to the character following the last character of the string
        /// This character acts as a placeholder, attempting to access it results in undefined behavior
        [[nodiscard]] constexpr const_iterator cend() const noexcept { return buffer_.cbegin() + size(); }
      ...
      public:
        ...
        /// \brief Returns an codepoint_iterator to the first character of the string
        constexpr codepoint_iterator begin_codepoint() noexcept { return codepoint_iterator{this, 0, 0}; }

        /// \brief Returns an codepoint_iterator to the first character of the string
        [[nodiscard]] constexpr const_codepoint_iterator begin_codepoint() const noexcept { return cbegin_codepoint(); }

        /// \brief Returns an codepoint_iterator to the first character of the string
        [[nodiscard]] constexpr const_codepoint_iterator cbegin_codepoint() const noexcept {
            return const_codepoint_iterator{this, 0, 0};
        }

        /// \brief Returns an codepoint_iterator to the character following the last character of the string
        /// This character acts as a placeholder, attempting to access it results in undefined behavior
        constexpr codepoint_iterator end_codepoint() noexcept {
            return codepoint_iterator{this, size_codepoints(), size()};
        }

        /// \brief Returns an codepoint_iterator to the character following the last character of the string
        /// This character acts as a placeholder, attempting to access it results in undefined behavior
        [[nodiscard]] constexpr const_codepoint_iterator end_codepoint() const noexcept {
            return const_codepoint_iterator{this, size_codepoints(), size()};
        }

        /// \brief Returns an codepoint_iterator to the character following the last character of the string
        /// This character acts as a placeholder, attempting to access it results in undefined behavior
        [[nodiscard]] constexpr const_codepoint_iterator cend_codepoint() const noexcept {
            return const_codepoint_iterator{this, size_codepoints(), size()};
        }

        /// \brief Returns a reverse codepoint_iterator to the first character of the reversed string
        constexpr reverse_codepoint_iterator rbegin_codepoint() noexcept {
            return std::reverse_iterator<codepoint_iterator>(end_codepoint());
        }

        /// \brief Returns a reverse codepoint_iterator to the first character of the reversed string
        [[nodiscard]] constexpr const_reverse_codepoint_iterator rbegin_codepoint() const noexcept {
            return std::reverse_iterator<const_codepoint_iterator>(end_codepoint());
        }

        /// \brief Returns a reverse codepoint_iterator to the first character of the reversed string
        [[nodiscard]] constexpr const_reverse_codepoint_iterator crbegin_codepoint() const noexcept {
            return std::reverse_iterator<const_codepoint_iterator>(end_codepoint());
        }

        /// \brief Returns a reverse codepoint_iterator to the character following the last character of the reversed
        /// string
        constexpr reverse_codepoint_iterator rend_codepoint() noexcept {
            return std::reverse_iterator<codepoint_iterator>(begin_codepoint());
        }

        /// \brief Returns a reverse codepoint_iterator to the character following the last character of the reversed
        /// string
        [[nodiscard]] constexpr const_reverse_codepoint_iterator rend_codepoint() const noexcept {
            return std::reverse_iterator<const_codepoint_iterator>(begin_codepoint());
        }

        /// \brief Returns a reverse codepoint_iterator to the character following the last character of the reversed
        /// string
        [[nodiscard]] constexpr const_reverse_codepoint_iterator crend_codepoint() const noexcept {
            return std::reverse_iterator<const_codepoint_iterator>(begin_codepoint());
        }

      public:
        /// \section Capacity
        /// All functions whose name are the same as the basic_string API refer to code units
        /// A number of extra functions are provided deal with code point capacity
        /// These branch into many functions as this can vary according to our expectations about
        /// the size of our future codepoints.

        /// \brief Checks if the string has no code units or code points
        [[nodiscard]] constexpr bool empty() const noexcept { return !size_.codeunit_size(); }

        /// \brief Returns the number of code units in the string
        [[nodiscard]] constexpr size_type size() const noexcept { return size_.codeunit_size(); }

        /// \brief Returns the number of code units in the string
        [[nodiscard]] constexpr size_type length() const noexcept { return size(); }

        /// \brief Returns the number of codepoints in the string
        [[nodiscard]] constexpr size_type size_codepoints() const noexcept { return size_.codepoint_size(); }

        /// \brief Returns the number of codepoints in the string
        [[nodiscard]] constexpr size_type length_codepoints() const noexcept { return size_codepoints(); }

        /// \brief Returns the maximum number of elements the string is able to hold
        [[nodiscard]] constexpr size_type max_size() const noexcept { return buffer_.max_size() - 1; }

        /// \brief Returns the number of code units the string has allocated space for
        /// This assumes the one code unit per code point in the elements to come
        /// In terms of code points, this is an upper bound capacity
        ...
        /// \brief Returns the buffer size, which is different from the capacity for strings
        /// This size is important to safely create string views that include the lookup table
        [[nodiscard]] constexpr size_type buffer_capacity() const { return buffer_.size(); }

        /// \brief Informs a std::basic_string object of a planned change in size
        /// Note that this adjusts for a new expected number of bytes / code units, not for a
        /// new expected number of codepoints or for a new expected buffer size.
        /// \param new_cap Number of bytes we plan to have in the string
        ...
        /// Note that this adjusts for a new expected number of bytes / code units, not for a
        /// new expected number of codepoints or for a new expected buffer size.
        /// \param new_cap Number of code units we plan to have in the string
        /// \param new_multibytes Number of multi-codeunit codepoints we plan to have in the string
        constexpr void reserve(size_type new_cap, size_type new_multibyte_entries_cap) {
            // Trivial case
            if constexpr (has_lookup_table) {
                if (new_cap < capacity()) {
                    const_lookup_table_type t = const_lookup_table();
            ...
            if constexpr (has_lookup_table) {
                ...
                const size_type expected_multibyte_size = std::max(t.size(), new_multibyte_entries_cap);
                const size_type expected_lookup_size =
                    const_lookup_table_type::size_for(new_cap, expected_multibyte_size);
                ...
                value_type *new_lookup_begin = buffer_.data() + buffer_.size() - lookup_table_size;
                value_type *old_lookup_begin = buffer_.data() + old_buffer_size - lookup_table_size;
                std::memmove(new_lookup_begin, old_lookup_begin, lookup_table_size);
            } else {
        ...
        /// Don't use this function if you are planning to insert more elements to this string
        constexpr void shrink_to_fit() {
            if constexpr (has_lookup_table) {
                ...
                const size_type new_buffer_size = size_.codeunit_size() + 1 + lookup_table_size;
                if (new_buffer_size == buffer_.size()) {
                    return;
                }

                // Move the lookup table right after the string buffer
                value_type *old_lookup_begin = buffer_.data() + buffer_.size() - lookup_table_size;
      ...
      public:
        ...
        /// \brief Inserts null-terminated character string pointed to by s at the position index.
        /// \throws std::out_of_range if index > size()
        ...
        /// \brief Inserts the characters in the range [s, s+count) at the position index.
        ...
        /// \brief Inserts a string, obtained by str.substr(index_str, count) at the position index
        /// \throws Throws std::out_of_range if index > size() or if index_str > str.size()
        ...
        constexpr basic_string &insert(size_type index, const basic_string &str, codepoint_index index_str,
            ...
            const_codepoint_iterator str_codepoint_last =
                count != npos ? str_codepoint_first + count : str.end_codepoint();
            const_iterator str_first = str.begin() + str_codepoint_first.byte_index();
            const_iterator str_last = str.begin() + str_codepoint_last.byte_index();
            insert(begin() + index, str_first, str_last);
            return *this;
        }
        ...
        /// \return An iterator which refers to the copy of the first inserted character or pos if no characters were
        /// inserted (count==0 or first==last or ilist.size()==0)
        template <typename Char> constexpr iterator insert(const_iterator pos, Char ch) { return insert(pos, 1, ch); }

        template <typename Char> constexpr codepoint_iterator insert(const_codepoint_iterator pos, Char ch) {
            return insert(pos, 1, ch);
        }

        /// \brief Inserts count copies of character ch before the element (if any) pointed by pos
        /// \return An iterator which refers to the copy of the first inserted character or pos if no characters were
        /// inserted (count==0 or first==last or ilist.size()==0)
        template <typename Char> constexpr iterator insert(const_iterator pos, size_type count, Char ch) {
            ...
            lookup_table_type t = lookup_table();
            size_type new_multibyte_cap = t.size() + (codeunits_per_codepoint == 1 ? 0 : count);
            ...
            std::memmove(new_pos.base(), buffer_.data() + pos_idx, size_.codeunit_size() - pos_idx);
        ...
        /// \brief Inserts characters from the range [first, last) before the element (if any) pointed by pos.
        ...
        /// \brief Inserts elements from initializer list ilist before the element (if any) pointed by pos
        /// \return An iterator which refers to the copy of the first inserted character or pos if no characters were
        /// inserted (count==0 or first==last or ilist.size()==0)
        template <typename Char> constexpr iterator insert(const_iterator pos, std::initializer_list<Char> ilist) {
            return insert(pos, ilist.begin(), ilist.end());
        }

        template <typename Char>
        constexpr codepoint_iterator insert(const_codepoint_iterator pos, std::initializer_list<Char> ilist) {
            const size_type pos_offset = pos.index();
        ...
        /// then inserts the elements from sv before the element (if any) pointed by pos, as if by
        /// insert(pos, sv.data(), sv.size()).
        /// This overload participates in overload resolution only if std::is_convertible_v<const T&,
        /// std::basic_string_view<CharT, Traits>> is true and std::is_convertible_v<const T&, const CharT*> is false
        /// \throws std::out_of_range if index > size()
        template <class T, std::enable_if_t<is_api_string_view_v<T>, int> = 0>
        ...
        template <class T, std::enable_if_t<is_api_string_view_v<T>, int> = 0>
        constexpr basic_string &insert(codepoint_index pos, const T &t) {
            std::basic_string_view<typename T::value_type, std::char_traits<typename T::value_type>> sv(t);
        ...
        /// inserts, before the element (if any) pointed by pos, the characters from the subview [index_str,
        /// index_str+count) of sv. If the requested subview lasts past the end of sv, or if count == npos, the
        /// resulting subview is [index_str, sv.size()). If index_str > sv.size(), or if index > size(),
        /// std::out_of_range is thrown. This overload participates in overload resolution only if
        ...
        /// \throws std::out_of_range if index > size() or if index_str > sv.size()
        template <class T, std::enable_if_t<is_api_string_view_v<T>, int> = 0>
        ...
        constexpr basic_string &erase(codepoint_index index = 0, codepoint_index count = codepoint_index(npos)) {
            ...
            codepoint_iterator last_codepoint = first_codepoint + count_codepoint;
        ...
        constexpr codepoint_iterator erase(const_codepoint_iterator position) {
        ...
        constexpr iterator erase(const_iterator first, const_iterator last) {
            ...
            *new_code_unit_end_pos = '\0';
            ...
            if constexpr (has_lookup_table) {
                ...
                // as all multibytes in that range will now point to a difference string index
        ...
        constexpr codepoint_iterator erase(const_codepoint_iterator first, const_codepoint_iterator last) {
        ...
        template <typename Char> constexpr void push_back(Char ch) { insert(end(), 1, ch); }
        ...
        constexpr void pop_back_codepoint() { erase(end_codepoint() - 1); }

        /// \brief Appends count copies of character ch
        template <typename Char> constexpr basic_string &append(size_type count, Char ch) {
            insert(end(), count, ch);
            return *this;
        }
        ...
        /// [pos, size()). \throws If pos > str.size(), std::out_of_range is thrown.
        constexpr basic_string &append(const basic_string &str, size_type pos, size_type count = npos) {
            insert(end(), str.begin() + pos, str.begin() + pos + std::min(count, str.size() - pos));
            return *this;
        }

        constexpr basic_string &append(const basic_string &str, codepoint_index pos,
                                       codepoint_index count = codepoint_index(npos)) {
            const_codepoint_iterator first_codepoint = str.cbegin_codepoint() + pos;
            const_codepoint_iterator last_codepoint =
                first_codepoint + std::min(count.value_of(), str.size_codepoints() - pos);
            insert(end(), str.cbegin() + first_codepoint.byte_index(), str.cbegin() + last_codepoint.byte_index());
            return *this;
        }

        /// \brief Appends characters in the range [s, s + count).
        /// This range can contain null characters
        template <typename Char> constexpr basic_string &append(const Char *s, size_type count) {
        ...
        /// The length of the string is determined by the first null character using Traits::length(s)
        template <typename Char> constexpr basic_string &append(const Char *s) {
        ...
        template <typename Char> constexpr basic_string &append(std::initializer_list<Char> ilist) {
            insert(end(), ilist);
            return *this;
        }
        ...
        /// Implicitly converts t to a string view sv as if by std::basic_string_view<CharT, Traits> sv = t;, then
        /// appends all characters from sv as if by append(sv.data(), sv.size()). This overload participates in overload
        /// resolution only if std::is_convertible_v<const T&, std::basic_string_view<CharT, Traits>> is true and
        /// std::is_convertible_v<const T&, const CharT*> is false.
        template <class T, std::enable_if_t<is_api_string_view_v<T>, int> = 0>
        ...
        /// \brief Appends characters from the a string view
        /// Implicitly converts t to a string view sv as if by std::basic_string_view<CharT, Traits> sv = t;, then
        /// appends the characters from the subview [pos, pos+count) of sv. If the requested subview extends past the
        /// end of sv, or if count == npos, the appended subview is [pos, sv.size()). If pos >= sv.size(),
        /// std::out_of_range is thrown. This overload participates in overload resolution only if
        /// std::is_convertible_v<const T&, std::basic_string_view<CharT, Traits>> is true and
        /// std::is_convertible_v<const T&, const CharT*> is false
        template <class T, std::enable_if_t<is_api_string_view_v<T>, int> = 0>
        ...
        /// \brief Appends the null-terminated character string pointed to by s.
        template <typename Char> constexpr basic_string &operator+=(const Char *s) { return append(s); }
        ...
        /// This functions cannot be implemented as string_view::starts_with because
        /// it might require conversions from code points to code units
        template <typename RhsChar, typename RhsTraits>
        constexpr bool starts_with(std::basic_string_view<RhsChar, RhsTraits> sv) const noexcept {
            if (empty()) {
                return sv.empty();
            }
            if (sv.empty()) {
            ...
            if constexpr (!is_same_utf_encoding_v<value_type, RhsChar>) {
                ...
                return std::equal(begin(), begin() + sv.size(), sv.begin(), sv.end());
            }
        }
        ...
        /// This functions cannot be implemented as string_view::starts_with because
        /// it might require conversions from code points to code units
        template <typename Char> constexpr bool starts_with(Char c) const noexcept {
            if (empty()) {
                return false;
            }
        ...
        /// \brief Checks if the string begins with a null-terminated character string s.
        /// This functions cannot be implemented as string_view::starts_with because
        /// it might require conversions from code points to code units
        template <typename Char> constexpr bool starts_with(const Char *s) const {
            ...
            if constexpr (!is_same_utf_encoding_v<value_type, Char>) {
                ...
                return std::equal(begin(), begin() + other_size, s, s + other_size);
            }
        }

        /// \brief Checks if the string end with a string view sv
        /// This functions cannot be implemented as string_view::ends_with because
        /// it might require conversions from code points to code units
        template <typename RhsChar, typename RhsTraits>
        constexpr bool ends_with(std::basic_string_view<RhsChar, RhsTraits> sv) const noexcept {
            if (empty()) {
                return sv.empty();
            }
            if (sv.empty()) {
            ...
            if constexpr (!is_same_utf_encoding_v<value_type, RhsChar>) {
                ...
                return std::equal(rbegin(), rbegin() + sv.size(), sv.rbegin(), sv.rend());
            }
        }
        ...
        /// This functions cannot be implemented as string_view::ends_with because
        /// it might require conversions from code points to code units
        template <typename Char> constexpr bool ends_with(Char c) const noexcept {
            if (empty()) {
                return false;
            }
            if constexpr (!is_same_utf_encoding_v<Char, value_type>) {
        ...
        /// \brief Checks if the string end with a null-terminated character string s.
        /// This functions cannot be implemented as string_view::ends_with because
        /// it might require conversions from code points to code units
        template <typename Char> constexpr bool ends_with(const Char *s) const {
            std::basic_string_view<Char> sv(s, std::char_traits<Char>::length(s));
            return ends_with(sv);
        }

        /// \brief Checks if the string contains with a string view s as substring
        /// This functions cannot be implemented as string_view::contains because
        /// it might require conversions from code points to code units
        template <typename RhsChar, typename RhsTraits>
        constexpr bool contains(std::basic_string_view<RhsChar, RhsTraits> sv) const noexcept {
            return find(sv) != npos;
        }

        /// \brief Checks if the string contains with a single character c as substring
        /// This functions cannot be implemented as string_view::contains because
        /// it might require conversions from code points to code units
        template <typename Char> constexpr bool contains(Char c) const noexcept { return find(c) != npos; }

        /// \brief Checks if the string contains with a null-terminated character string s as substring
        /// This functions cannot be implemented as string_view::contains because
        /// it might require conversions from code points to code units
        template <typename Char> constexpr bool contains(const Char *s) const { return find(s) != npos; }
        ...
        constexpr basic_string &replace(const_codepoint_iterator first, const_codepoint_iterator last,
                                        const basic_string &str) {
            iterator first_code_unit = begin() + first.byte_index();
            iterator last_code_unit = begin() + last.byte_index();
            return replace(first_code_unit, last_code_unit, str.begin(), str.end());
        }
        ...
        constexpr basic_string &replace(size_type pos, size_type count, const basic_string &str, codepoint_index pos2,
            ...
            const_codepoint_iterator other_last_code_point =
                other_first_code_point + std::min(count2.value_of(), str.size_codepoints() - pos2);
            const_iterator other_first_code_unit = str.begin() + other_first_code_point.byte_index();
            const_iterator other_last_code_unit = str.begin() + other_last_code_point.byte_index();
            return replace(begin() + pos, begin() + pos + std::min(count, size() - pos), other_first_code_unit,
        ...
        constexpr basic_string &replace(codepoint_index pos, codepoint_index count, const basic_string &str,
            ...
            codepoint_iterator last_code_point =
                begin_codepoint() + pos + std::min(count.value_of(), size_codepoints() - pos);
            iterator first_code_unit = begin() + first_code_point.byte_index();
            iterator last_code_unit = begin() + last_code_point.byte_index();
            return replace(first_code_unit, last_code_unit, str.begin() + pos2,
                           str.begin() + pos2 + std::min(count2, str.size() - pos2));
        }
        ...
        constexpr basic_string &replace(codepoint_index pos, codepoint_index count, const basic_string &str,
            ...
            iterator first_code_unit = begin() + first_code_point.byte_index();
            iterator last_code_unit = begin() + last_code_point.byte_index();
            const_codepoint_iterator other_first_code_point = str.begin_codepoint() + pos;
            const_codepoint_iterator other_last_code_point =
                str.begin_codepoint() + pos + std::min(count2.value_of(), str.size_codepoints() - pos2);
            const_iterator other_first_code_unit = str.begin() + other_first_code_point.byte_index();
            const_iterator other_last_code_unit = str.begin() + other_last_code_point.byte_index();
            return replace(first_code_unit, last_code_unit, other_first_code_unit, other_last_code_unit);
        }
        ...
        constexpr basic_string &replace(const_iterator first, const_iterator last, InputIt first2, InputIt last2) {
            ...
            // However, we still need to implement the more specific case where the ranges have the same size
            // If that's the case, we can avoid allocating/deallocating space for elements
            // However, we still need to check if they have the same multibyte pattern because, otherwise,
            // we still need to update the lookup table.
            constexpr bool is_input_iterator =
                std::is_same_v<typename std::iterator_traits<InputIt>::iterator_category, std::input_iterator_tag>;
        ...
        template <class InputIt>
        constexpr basic_string &replace(const_codepoint_iterator first, const_codepoint_iterator last, InputIt first2,
                                        InputIt last2) {
            iterator first_code_unit = begin() + first.byte_index();
            iterator last_code_unit = begin() + last.byte_index();
        ...
        constexpr basic_string &replace(codepoint_index pos, codepoint_index count, const Char *cstr,
            ...
            codepoint_iterator first_code_point = begin_codepoint() + pos;
            codepoint_iterator last_code_point =
                begin_codepoint() + pos + std::min(count.value_of(), size_codepoints() - pos.value_of());
            iterator first_code_unit = begin() + first_code_point.byte_index();
            iterator last_code_unit = begin() + last_code_point.byte_index();
        ...
        /// \brief Replaces the part of the string with the range [cstr, cstr+count2)
        template <typename Char>
        constexpr basic_string &replace(const_iterator first, const_iterator last, const Char *cstr, size_type count2) {
            return replace(first, last, cstr, cstr + count2);
        }

        template <typename Char>
        constexpr basic_string &replace(const_codepoint_iterator first, const_codepoint_iterator last, const Char *cstr,
                                        size_type count2) {
            iterator first_code_unit = begin() + first.byte_index();
            iterator last_code_unit = begin() + last.byte_index();
            return replace(first_code_unit, last_code_unit, cstr, cstr + count2);
        ...
        constexpr basic_string &replace(codepoint_index pos, codepoint_index count, const Char *cstr) {
            ...
            iterator first_code_unit = begin() + first_code_point.byte_index();
            iterator last_code_unit = begin() + last_code_point.byte_index();
            return replace(first_code_unit, last_code_unit, cstr, std::char_traits<Char>::length(cstr));
        }

        /// \brief Replaces the part of the string with cstr
        template <typename Char>
        constexpr basic_string &replace(const_iterator first, const_iterator last, const Char *cstr) {
            return replace(first, last, cstr, std::char_traits<Char>::length(cstr));
        }
        ...
        constexpr basic_string &replace(const_codepoint_iterator first, const_codepoint_iterator last,
            ...
            iterator first_code_unit = begin() + first.byte_index();
            iterator last_code_unit = begin() + last.byte_index();
            return replace(first_code_unit, last_code_unit, ilist.begin(), ilist.end());
        }

        /// \brief Replaces the part of the string with (convertible to) string view t
        template <class T, std::enable_if_t<is_api_string_view_v<T>, int> = 0>
        ...
        template <class T, std::enable_if_t<is_api_string_view_v<T>, int> = 0>
        constexpr basic_string &replace(const_codepoint_iterator first, const_codepoint_iterator last, const T &t) {
            std::basic_string_view sv(t);
            iterator first_code_unit = begin() + first.byte_index();
            iterator last_code_unit = begin() + last.byte_index();
            return replace(first_code_unit, last_code_unit, sv.begin(), sv.end());
        ...
        /// \brief Replaces the part of the string with substr of (convertible to) string view t
        template <class T, std::enable_if_t<is_api_string_view_v<T>, int> = 0>
        ...
        /// The returned string is constructed as if by basic_string(data()+pos, count), which implies that the returned
        /// string's allocator will be default-constructed — the new allocator might not be a copy of
        /// this->get_allocator()
        [[nodiscard]] constexpr basic_string substr(size_type pos = 0, size_type count = npos) const {
            return basic_string(data() + pos, count);
        }

        [[nodiscard]] constexpr basic_string substr(codepoint_index pos = 0,
            ...
            const_codepoint_iterator last_code_point =
                first_code_point + std::min(count.value_of(), size_codepoints() - pos.value_of());
            return basic_string(data() + first_code_point.byte_index(), data() + last_code_point.byte_index());
        }

        /// \brief Copies a substring [pos, pos+count) to character string pointed to by dest.
        /// If the requested substring lasts past the end of the string, or if count == npos, the copied substring is
        ...
        template <typename Char> constexpr size_type copy(Char *dest, size_type count, size_type pos = 0) const {
            ...
            if constexpr (sizeof(Char) == sizeof(value_type)) {
                ...
                const_codepoint_iterator first_cp = find_codepoint(first - begin());
                const_codepoint_iterator last_cp = find_codepoint(last - begin());
        ...
        constexpr size_type copy(Char *dest, codepoint_index count, codepoint_index pos = codepoint_index(0)) const {
            ...
            const const_codepoint_iterator last_cp =
                first_cp + std::min(count.value_of(), size_codepoints() - pos.value_of());
            const const_iterator first = find_codeunit(codepoint_index(first_cp - begin_codepoint()));
            const const_iterator last = find_codeunit(codepoint_index(last_cp - begin_codepoint()));
        ...
        constexpr void resize(codepoint_index count) { resize(count, '\0'); }

        /// \brief Resizes the string to contain count characters
        /// \note This can lead to malformed utf strings.
        template <typename Char> constexpr void resize(size_type count, Char ch) {
            if (count < size()) {
                downsize(count);
            } else if (count > size()) {
                upsize(count, ch);
        ...
        template <typename Char> constexpr void resize(codepoint_index count, Char ch) {
        ...
        /// The behavior is undefined if Allocator does not propagate on swap and the allocators of *this and other are
        /// unequal
      ...
      public:
        ...
        [[nodiscard]] iterator find_codeunit(codepoint_index index) {
            // Find codepoint
        ...
        [[nodiscard]] const_iterator find_codeunit(codepoint_index index) const {
            return (const_cast<basic_string *>(this))->find_codeunit(index);
        }

        /// \brief Find a code point iterator for a given code unit
        /// If this is a continuation code unit, this function might return a codepoint
        ...
        [[nodiscard]] codepoint_iterator find_codepoint(size_type code_unit_index) {
            ...
            const_lookup_table_type t = const_lookup_table();
            auto first_multibyte_after = t.lower_bound_codeunit(code_unit_index);
            const size_type first_multibyte_after_codepoint = first_multibyte_after.codepoint_index();
            // Calculate their distance
            const size_type multibyte_code_unit_index = first_multibyte_after.byte_index();
            const size_type byte_distance = multibyte_code_unit_index - code_unit_index;
            if (byte_distance == 0) {
        ...
        [[nodiscard]] const_codepoint_iterator find_codepoint(size_type code_unit_index) const {
            return (const_cast<basic_string *>(this))->find_codepoint(code_unit_index);
        }
        ...
        /// - for a non-empty substring, if pos >= size(), the function always returns npos
        /// \return Position of the first character of the found substring or npos if no such substring is found
        template <typename Char> constexpr size_type find(const Char *s, size_type pos, size_type count) const {
            if (count == 0) {
                // return pos
                return pos;
            }
            ...
            if constexpr (same_encoding) {
                ...
                const_codepoint_iterator it = std::search(this_cp_first, this_cp_last, s, s + count);
                if (it != end_codepoint()) {
        ...
        /// - for a non-empty substring, if pos >= size(), the function always returns npos
        /// \return Position of the first character of the found substring or npos if no such substring is found
        template <typename Char> constexpr size_type find(const Char *s, size_type pos = 0) const {
            return find(s, pos, std::char_traits<Char>::length(s));
        }

        /// \brief Finds the first character ch (treated as a single-character substring by the formal rules below)
        /// - a substring can be found only if pos <= size() - str.size()
        /// - an empty substring is found at pos if and only if pos <= size()
        /// - for a non-empty substring, if pos >= size(), the function always returns npos
        /// \return Position of the first character of the found substring or npos if no such substring is found
        ...
        ///         - If searching for an empty string, returns pos unless pos > size(), in which case returns size()
        ///         - if size() is zero, npos is always returned.
        template <typename Char> constexpr size_type rfind(const Char *s, size_type pos, size_type count) const {
            if (count == 0) {
                // return pos
                return pos;
            }
        ...
        ///         - If searching for an empty string, returns pos unless pos > size(), in which case returns size()
        ///         - if size() is zero, npos is always returned.
        template <typename Char> constexpr size_type rfind(const Char *s, size_type pos = npos) const {
        ...
        /// \brief Finds the first character equal to one of the characters in the given character sequence
        ...
        /// If the character is not present in the interval, npos will be returned
        /// \return Position of the found character or npos if no such character is found
        template <typename Char> constexpr size_type find_first_of(const Char *s, size_type pos = 0) const {
        ...
        /// If the character is not present in the interval, npos will be returned.
        /// \return Position of the found character or npos if no such character is found
        template <typename Char> constexpr size_type find_first_not_of(const Char *s, size_type pos = 0) const {
        ...
        /// If the character is not present in the interval, npos will be returned
        /// \return Position of the found character or npos if no such character is found.
        template <typename Char> constexpr size_type find_last_of(const Char *s, size_type pos, size_type count) const {
        ...
        /// If the character is not present in the interval, npos will be returned
        /// \return Position of the found character or npos if no such character is found.
        template <typename Char> constexpr size_type find_last_of(const Char *s, size_type pos = npos) const {
        ...
        /// If the character is not present in the interval, npos will be returned
        /// \return Position of the found character or npos if no such character is found.
        [[nodiscard]] constexpr size_type find_last_not_of(const basic_string &str,
        ...
        /// If the character is not present in the interval, npos will be returned
        /// \return Position of the found character or npos if no such character is found.
        template <typename Char> constexpr size_type find_last_not_of(const Char *s, size_type pos = npos) const {
      ...
      public:
        ...
        template <class InputIt> constexpr int compare(InputIt first, InputIt last) const noexcept {
            using input_value_type = std::decay_t<typename std::iterator_traits<InputIt>::value_type>;
            ...
            while (this_it != this_end && first != last && *first) {
                if constexpr (is_same_utf_encoding_v<value_type, input_value_type>) {
                    ...
                    while (this_it != this_end && encoded_first != encoded_last && *encoded_first) {
      ...
      public:
        ...
        /// std::allocator_traits<Alloc>::select_on_container_copy_construction(lhs.get_allocator()) \return A string
        /// containing characters from lhs followed by the characters from rhs
        ...
        template <typename Char> friend constexpr basic_string operator+(const basic_string &lhs, const Char *rhs) {
            basic_string res(lhs);
            res += rhs;
            return res;
        }
        ...
        /// std::allocator_traits<Alloc>::select_on_container_copy_construction(lhs.get_allocator())
        /// \return A string containing characters from lhs followed by the characters from rhs
        template <typename Char> friend constexpr basic_string operator+(const basic_string &lhs, Char rhs) {
            basic_string res(lhs);
            res += rhs;
            return res;
        }
        ...
        /// \brief Returns a string containing characters from lhs followed by the characters from rhs
        /// The allocator used for the result is
        /// std::allocator_traits<Alloc>::select_on_container_copy_construction(rhs.get_allocator()) \return A string
        /// containing characters from lhs followed by the characters from rhs
        template <typename Char> friend constexpr basic_string operator+(const Char *lhs, const basic_string &rhs) {
            basic_string res(lhs);
            res += rhs;
            return res;
        }
        ...
        /// std::allocator_traits<Alloc>::select_on_container_copy_construction(rhs.get_allocator()) \return A string
        /// containing characters from lhs followed by the characters from rhs
        template <typename Char> friend constexpr basic_string operator+(Char lhs, const basic_string &rhs) {
        ...
        /// \brief Returns a string containing characters from lhs followed by the characters from rhs
        /// The allocator used for the result is lhs.get_allocator()
        /// \return A string containing characters from lhs followed by the characters from rhs
        ...
        /// The allocator used for the result is lhs.get_allocator()
        /// \return A string containing characters from lhs followed by the characters from rhs
        template <typename Char> friend constexpr basic_string operator+(basic_string &&lhs, const Char *rhs) {
            basic_string res(std::move(lhs));
            res += rhs;
            return res;
        }
        ...
        /// The allocator used for the result is lhs.get_allocator()
        /// \return A string containing characters from lhs followed by the characters from rhs
        template <typename Char> friend constexpr basic_string operator+(basic_string &&lhs, Char rhs) {
            basic_string res(std::move(lhs));
            res += rhs;
            return res;
        }
        ...
        /// \brief Returns a string containing characters from lhs followed by the characters from rhs
        /// The allocator used for the result is rhs.get_allocator()
        /// \return A string containing characters from lhs followed by the characters from rhs
        ...
        /// The allocator used for the result is rhs.get_allocator()
        /// \return A string containing characters from lhs followed by the characters from rhs
        template <typename Char> friend constexpr basic_string operator+(const Char *lhs, basic_string &&rhs) {
            basic_string res(lhs);
            res += std::move(rhs);
            return res;
        }
        ...
        /// The allocator used for the result is rhs.get_allocator()
        /// \return A string containing characters from lhs followed by the characters from rhs
        template <typename Char> friend constexpr basic_string operator+(Char lhs, basic_string &&rhs) {
            basic_string res(1, lhs);
            res += std::move(rhs);
            return res;
        }
        ...
        template <class U> friend constexpr typename basic_string::size_type erase(basic_string &c, const U &value) {
        ...
        /// \return The number of erased elements
        template <class Pred> friend constexpr typename basic_string::size_type erase_if(basic_string &c, Pred pred) {
        ...
        /// \throws may throw std::ios_base::failure if an exception is thrown during output
        friend std::ostream &operator<<(std::ostream &os, const basic_string &str) {
            ...
            // 1) Behaves as a FormattedOutputFunction
            //   a) If str.size() is not less than os.width(), uses the range [str.begin(), str.end()) as-is
            //   b) Otherwise, if (os.flags() & ios_base::adjustfield) == ios_base::left, places os.width()-str.size()
            //   copies of the os.fill() character after the character sequence c) Otherwise, places
            //   os.width()-str.size() copies of the os.fill() character before the character sequence
        ...
        /// \see https://en.cppreference.com/w/cpp/named_req/FormattedInputFunction
        /// \throws may throw std::ios_base::failure if no characters are extracted from is (e.g the stream is at end of
        /// file, or consists of whitespace only), or if an exception is thrown during input.
        friend std::istream &operator>>(std::istream &is, basic_string &str) {
            // 1) After constructing and checking the sentry object, which may skip leading whitespace,
            //   a) first clears str with str.erase()
            //   b) then reads characters from is and appends them to str as if by str.append(1, c), until one of the
            //   following conditions becomes true:
            //      (i) N characters are read, where N is is.width() if is.width() > 0, otherwise N is str.max_size()
            //      (ii) the end-of-file condition occurs in the stream is
            //      (iii) std::isspace(c,is.getloc()) is true for the next character c in is (this whitespace character
            //      remains in the input stream).
            // 2) If no characters are extracted then std::ios::failbit is set on is, which may throw
            // std::ios_base::failure 3) Finally, calls is.width(0) to cancel the effects of std::setw, if any
        ...
        /// \see https://en.cppreference.com/w/cpp/named_req/UnformattedInputFunction
        /// When consuming whitespace-delimited input (e.g. int n; std::cin >> n;) any whitespace that follows,
        /// including a newline character, will be left on the input stream. Then when switching to line-oriented input,
        /// the first line retrieved with getline will be just that whitespace. In the likely case that this is unwanted
        /// behaviour, possible solutions include:
        /// - An explicit extraneous initial call to getline
        /// - Removing consecutive whitespace with std::cin >> std::ws
        /// - Ignoring all leftover characters on the line of input with
        /// cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        friend std::istream &getline(std::istream &is, basic_string &str, CharT delim) {
            // After constructing and checking the sentry object, performs the following
            // 1) Calls str.erase()
            // 2) Extracts characters from input and appends them to str until one of the following occurs (checked in
            // the order listed)
            //   a) end-of-file condition on input, in which case, getline sets eofbit
            //   b) the next available input character is delim, as tested by Traits::eq(c, delim), in which case the
            //   delimiter character is extracted from input, but is not appended to str c) str.max_size() characters
            //   have been stored, in which case getline sets failbit and returns
        ...
        /// Behaves as UnformattedInputFunction, except that input.gcount() is not affected.
        /// \see https://en.cppreference.com/w/cpp/named_req/UnformattedInputFunction
        friend std::istream &getline(std::istream &input, basic_string &str) {
      ...
      private:
        ...
        /// Using our inline vector brings the cache optimizations to the string type
        using char_vector_type = small::vector<value_type, N + 1>;

        /// \brief Size it takes to store the null '\0' char
        /// Of course we could just write 1 in the code, but this gives us better semantics
        ...
        /// All utf8/utf16 strings have a lookup table because they have more than one code unit per code point.
        /// If the string ends up having no multiple code units per codepoint, the lookup table takes only one byte.
        static constexpr bool store_codepoint_size = is_utf8_string || is_utf16_string;
      ...
      private:
        ...
        constexpr lookup_table_type lookup_table() noexcept {
            return lookup_table_type(buffer_.data() + size_.codeunit_size() + 1,
                                     buffer_.size() - size_.codeunit_size() - 1, this->operator string_view_type());
        }

        [[nodiscard]] constexpr const_lookup_table_type const_lookup_table() const noexcept {
            return const_lookup_table_type(buffer_.data() + size_.codeunit_size() + 1,
        ...
        /// If the input is wide with more than one code unit per code points, then count
        /// will refer to code points, as to avoid malformed strings
        template <typename Char> constexpr void upsize(size_type count, Char ch) {
            assert(count > size());
            ...
            reserve(size() + code_unit_increment, ct.size() + (code_units_per_char > 1 ? code_point_increment : 0));
            ...
            if constexpr (!is_same_utf_encoding_v<value_type, Char>) {
                if (code_units_per_char == 1) {
                    ...
                    if (code_units_per_char < code_unit_increment) {
                        ...
                        // Include a malformed code point if we need to complete the buffer
                        value_type *fill_end_codepoint = fill_begin + code_point_increment * code_units_per_char;
                        const size_type extra_code_units = fill_end - fill_end_codepoint;
                        if (extra_code_units > 0) {
                            std::memcpy(fill_end_codepoint, fill_begin, extra_code_units);
                        }
                    }
                    // Fill the lookup table
                    if constexpr (has_lookup_table) {
                        ...
                        for (size_type i = 0; i < code_point_increment; ++i) {
                            t.insert_or_assign(multibyte_index, multibyte_code_unit_index, multibyte_codepoint_index);
                            ++multibyte_index;
        ...
        /// \brief Count the number of code points in a range of code units [first, last)
        /// \param first First code unit
        /// \param last  Last+1 code unit
        /// \return Number of code points between these two code units
        constexpr size_type size_codepoints(const_iterator first, const_iterator last) {
            if (first >= last) {
                return 0;
            }
            if constexpr (has_lookup_table) {
                ...
                if (!range_contains_multibyte_code_units) {
                    ...
                    auto multibyte_last_it =
                        multibyte_first_it == t.end() ? multibyte_first_it : t.lower_bound_codeunit(last_idx);
                    // Get distance in codepoints between them
                    const size_type mb_last_idx =
                        multibyte_first_it == multibyte_last_it ? mb_first_idx : multibyte_last_it.codepoint_index();
                    const size_type multibyte_codepoint_distance = mb_last_idx - mb_first_idx;
                    // Adjust the distance to:
                    // 1) include [first,first_multibyte] range
                    // 2) remove [last,last_multibyte] range
                    const size_type range_first = multibyte_first_it.byte_index() - first_idx;
                    const size_type range_last = multibyte_last_it.byte_index() - last_idx;
                    return multibyte_codepoint_distance + range_first - range_last;
                }
            } else {
        ...
        constexpr void downsize(size_type count) {
            assert(count < size());
            const size_type cur_size = size();
            const size_type code_unit_decrement = cur_size - count;
            const size_type code_point_decrement = size_codepoints(begin() + count, end());

            // Remove elements from the lookup table
            if constexpr (has_lookup_table) {
                lookup_table_type t = lookup_table();
                // Find first entry indexing an index >= count
                typename lookup_table_type::iterator it = t.lower_bound_codeunit(count);
        ...
        ///
        /// We use this function to update the multibyte indexes relative to this
        /// range after any operation that affects its elements.
        ///
        /// Very often, there's nothing to do here, so we want doing any work
        /// we don't have to.
        ///
        /// In the general case, when there is work to do, resizing the entries is
        /// very expensive, so we (i) count the number of multibyte entries first,
        /// (ii) resize the table once, and (iii) iterate again to set the values.
        /// \param first First element that has been updated
        /// \param last Last + 1 element that has been updated
        void index_multibyte_codepoints(const_iterator first, const_iterator last) {
            ...
            const lut_iterator first_multibyte_after_first = t.lower_bound_codeunit(first_idx);
            const size_type first_multibyte_after_first_idx = first_multibyte_after_first.byte_index();
            const bool lut_has_multibytes_in_range = first_multibyte_after_first_idx < last_idx;
            const lut_iterator first_multibyte_after_last =
                lut_has_multibytes_in_range ? t.lower_bound_codeunit(last_idx) : first_multibyte_after_first;
            const size_type lut_multibytes_in_range = first_multibyte_after_last - first_multibyte_after_first;

            // Count how many multibyte codepoints we have to index in the new input range
            ...
            // Find properties of first multibyte entry before resizing the table
            // We need the end() iterator to refer to the old table at this point
            size_type codeunit_idx = first_idx;
            size_type codepoint_idx =
                first_multibyte_after_first.codepoint_index() - (first_multibyte_after_first_idx - first_idx);
            size_type multibyte_idx = first_multibyte_after_first.multibyte_index();

            // Resize the lookup table
            const size_type table_old_size = t.size();
            if (input_multibytes_in_range > lut_multibytes_in_range) {
                const size_type table_increment = input_multibytes_in_range - lut_multibytes_in_range;
                const size_type table_new_size = table_old_size + table_increment;
                t.resize(table_new_size);
            } else if (input_multibytes_in_range < lut_multibytes_in_range) {
                const size_type table_decrement = lut_multibytes_in_range - input_multibytes_in_range;
                const size_type table_new_size = table_old_size - table_decrement;
                t.resize(table_new_size);
            }
        ...
        constexpr size_type find_first_of_common(const Char *s, size_type pos, size_type count) const {
            ...
            if constexpr (same_encoding) {
                ...
                const_codepoint_iterator it = std::find_if(this_cp_first, this_cp_last, [&](auto wide_ch) {
                    if constexpr (look_for_existence) {
        ...
        template <bool look_for_existence, typename Char>
        constexpr size_type find_last_of_common(const Char *s, size_type pos, size_type count) const {
            constexpr bool same_encoding = sizeof(Char) == sizeof(value_type);
            if (pos >= size()) {
                pos = size() - 1;
            }
            if constexpr (same_encoding) {
                auto it = begin() + pos;
                bool it_matches = look_for_existence ? std::find(s, s + count, *it) != s + count
                ...
                while (it != begin()) {
                    --it;
                    it_matches = look_for_existence ? std::find(s, s + count, *it) != s + count
                                                    : std::find(s, s + count, *it) == s + count;
                    if (it_matches) {
                        return it - begin();
                    }
                ...
                bool cp_match = look_for_existence ? std::find(s, s + count, *it) != s + count
                                                   : std::find(s, s + count, *it) == s + count;
                if (cp_match) {
                    return it.byte_index();
                }
                while (it != cp_begin) {
                    --it;
                    cp_match = look_for_existence ? std::find(s, s + count, *it) != s + count
                                                  : std::find(s, s + count, *it) == s + count;
                    if (cp_match) {
                        return it.byte_index();
                    }
      ...
      private:
        ...
        /// This acts as a buffer, because this vector is always larger than the string, to allocate
        /// the '\0' char and the lookup table
        ...
        struct internal_size_multibyte {
          ...
          private:
            /// \brief Buffer size allocated for the string without the \0 and or the lookup table (see (i) above)
            /// i.e.: Where the string ends in the buffer
            /// i.e.: What basic_string::size() returns
            size_type codeunit_size_{0};

            /// \brief The number of codepoints in this string
            /// If the string has no multibyte codepoints, this is the same as codeunit_size_
    ...
    inline small::string operator""_ss(const char *str, std::size_t len) { return small::string{str, len}; }

    /// \brief Interprets a signed integer value in the string str
    /// calls std::strtol(str.c_str(), &ptr, base) or std::wcstol(str.c_str(), &ptr, base)
    ...
    /// \brief Interprets a signed integer value in the string str
    /// calls std::strtoll(str.c_str(), &ptr, base) or std::wcstoll(str.c_str(), &ptr, base)
    /// \throws std::invalid_argument if no conversion could be performed
    /// \throws std::out_of_range if the converted value would fall out of the range of the result type or if the
    /// underlying function (std::strtol or std::strtoll) sets errno to ERANGE.
    ...
    /// calls std::strtoul(str.c_str(), &ptr, base) or std::wcstoul(str.c_str(), &ptr, base)
    /// \throws std::invalid_argument if no conversion could be performed
    /// \throws std::out_of_range if the converted value would fall out of the range of the result type or if the
    ...
    /// \brief Interprets an unsigned integer value in the string str
    /// calls std::strtoull(str.c_str(), &ptr, base) or std::wcstoull(str.c_str(), &ptr, base)
    /// \throws std::invalid_argument if no conversion could be performed
    /// \throws std::out_of_range if the converted value would fall out of the range of the result type or if the
    /// underlying function (std::strtoul or std::strtoull) sets errno to ERANGE.
    ...
    /// We use a string stream rather than the usual std::sprintf for this operation
    /// because it's safer. In any case, we should prefer from_chars / to_chars.
    template <typename T, std::enable_if_t<std::is_integral_v<T> || std::is_floating_point_v<T>, int> = 0>
    small::string to_string(T value) {
    ...
    /// This string class allows loose and malformed sequences of code units in order
    /// to be compatible with std::string and provide byte access.
    /// We assume only utf8 strings can be malformed at this point and that utf16/utf32
    /// can only have one code point per code unit, which is strictly false for utf16.
    template <typename CharT, size_t N, class Traits, typename WCharT, class Allocator, size_t CP_HINT_STEP,
              class SizeType>
    ...
    /// This allows us to create an inline vector of inline strings that we can memcpy
    template <typename CharT, size_t N, class Traits, typename WCharT, class Allocator, size_t CP_HINT_STEP,
              class SizeType>
    struct is_relocatable<small::basic_string<CharT, N, Traits, WCharT, Allocator, CP_HINT_STEP, SizeType>>
        : std::true_type {};
...
namespace std {
    ...
    template <typename CharT, size_t N, class Traits, typename WCharT, class Allocator, size_t CP_HINT_STEP,
              class SizeType>
    struct hash<small::basic_string<CharT, N, Traits, WCharT, Allocator, CP_HINT_STEP, SizeType>> {
        std::size_t operator()(const small::basic_string<CharT, N, Traits, WCharT, Allocator, CP_HINT_STEP, SizeType>
                                   &string) const noexcept {
            using value_type =
                typename small::basic_string<CharT, N, Traits, WCharT, Allocator, CP_HINT_STEP, SizeType>::value_type;
...
#endif // SMALL_STRING_H8_STRING_H
