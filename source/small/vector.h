...
#include "detail/iterator/iterator_type_traits.h"
#include "detail/iterator/pointer_wrapper.h"
#include "detail/traits/default_inline_storage.h"
#include "detail/traits/enable_allocator_from_this.h"
#include "detail/traits/is_range.h"
#include "detail/traits/is_relocatable.h"
...
namespace small {
    ...
    class vector : public enable_allocator_from_this<std::is_empty_v<Allocator>, Allocator> {
      ...
      public:
        ...
        static_assert(std::is_unsigned<size_t>::value, "Size type should be an unsigned integral type");
      ...
      public:
        ...
        static constexpr size_t heap_storage_size = pointer_size + size_type_size;

        /// \brief Size of a value
        static constexpr size_t value_size = sizeof(value_type);

        /// \brief How many values we can store in the space allocated for the heap
        static constexpr size_t inline_values_per_heap = heap_storage_size / value_size;
        ...
        static constexpr size_t min_inline_elements = std::max(inline_values_per_heap, min_reasonable_inline_elements);

        /// \brief Pointers per value
        static constexpr auto requested_inline_size = N;

        /// \brief Number of elements we should inline
        /// If the requested number is below the number we can fit in the pointer itself, use that instead
        static constexpr std::size_t num_inline_elements = std::max(min_inline_elements, requested_inline_size);
        ...
        static constexpr bool inline_storage_empty = sizeof(value_type) * num_inline_elements == 0;
        ...
        /// We don't really represent the std::allocator in the class because it has no members
        /// We can just recreate it at get_alloc()
        using internal_allocator_type = std::conditional_t<using_std_allocator, empty_type, allocator_type>;

        /// \brief Use memcpy to copy items
        /// If type is relocatable, we just use memcpy
        static constexpr bool relocate_use_memcpy = is_relocatable_v<T> && using_std_allocator;
      ...
      public:
        ...
        vector &operator=(vector &&rhs) noexcept(std::is_nothrow_move_constructible<value_type>::value &&
      ...
      public /* constructors */:
        ...
        constexpr explicit vector(const allocator_type &alloc) : vector(0, alloc) {}

        /// \brief Construct small array with size n
        /// Any of the n values should be constructed with new (p) value_type();
        constexpr explicit vector(size_type n, const allocator_type &alloc = allocator_type())
            : vector(
                  n, [&](void *p) { new (p) value_type(); }, alloc) {}
        ...
        constexpr vector(size_type n, const value_type &value, const allocator_type &alloc = allocator_type())
            : vector(
                  n, [&](void *p) { new (p) value_type(value); }, alloc) {}
        ...
        constexpr vector(std::initializer_list<value_type> il, const allocator_type &alloc = allocator_type())
        ...
        constexpr void swap(vector &rhs) noexcept(
            ...
            // Change allocators
            const bool should_swap = std::allocator_traits<allocator_type>::propagate_on_container_swap::value;
      ...
      public /* capacity */:
        ...
        [[nodiscard]] constexpr size_type max_size() const noexcept {
            if constexpr (!should_use_heap) {
                ...
                return std::min<size_type>(
                    std::allocator_traits<allocator_type>::max_size(enable_allocator_type::get_allocator()),
                    max_with_mask);
            }
        }
        ...
        [[nodiscard]] constexpr bool is_inline() const { return !is_external(); }

        /// \brief Reserve space for n elements
        /// We concentrate the logic to switch the variant types in these functions
        void reserve(size_type n) { make_size(n); }

        /// \brief Reserve space for n elements
        /// We concentrate the logic to switch the variant types in these functions
        /// The behaviour of this function might change
        void reserve() { shrink_to_fit(); }
      ...
      public /* element access */:
        ...
        constexpr T *data() noexcept { return this->is_external() ? data_.heap() : data_.buffer(); }
      ...
      public /* modifiers */:
        ...
        constexpr void push_back(value_type &&v) { emplace_back(std::move(v)); }
        ...
        constexpr void resize(size_type n, const value_type &v) {
            if (n < size()) {
                erase(begin() + n, end());
                return;
            }
      ...
      private:
        ...
        static size_type constexpr size_type_most_significant_bit_mask = size_type(1) << (sizeof(size_type) * 8 - 1);

        /// \brief A mask to identify if the array is external
        static size_type constexpr is_external_mask = should_use_heap ? size_type_most_significant_bit_mask : 0;

        /// \brief Identify if we are using the heap
        /// A mask to extract the size bits identifying whether the current vector it's external
        /// The two most significant digits can tell us if something is external / allocated in the heap
        [[nodiscard]] constexpr bool is_external() const { return is_external_mask & size_; }

        /// \brief Set the flag indicating the vector is external
        void set_external(bool b) {
            if (b) {
        ...
        [[nodiscard]] constexpr size_t get_unmasked_size() const { return size_ & clear_size_mask; }
        ...
        void make_size(size_type new_size, EmplaceFunc &&emplace_function, size_type new_emplaced_size) {
            assert(size() == capacity());
            make_size_internal<std::true_type>(new_size, std::forward<EmplaceFunc>(emplace_function),
                                               new_emplaced_size);
        }
        ...
        void move_to_uninitialized_emplace(value_type *begin, value_type *end, value_type *out, size_type pos,
        ...
        /// \brief Move the range first/last to uninitialized memory starting at out
        template <class T2 = value_type, std::enable_if_t<!std::is_trivially_copyable_v<T2>, int> = 0>
        void move_to_uninitialized(T *first, T *last, T *out) {
            std::size_t idx = 0;
            {
                // Call destructor for all elements we emplaced correctly if something fails
                auto rollback = make_guard([&] {
        ...
        constexpr size_type compute_new_size() const {
            ...
            const bool cannot_be_in_place = old_capacity < min_in_place_expansion / sizeof(T);
            if (cannot_be_in_place) {
                return capacity() * 2;
            }
            const bool very_very_large_already = capacity() > 4096 * 32 / sizeof(T);
        ...
        static void partially_uninitialized_copy(Iterator1 from, size_t from_size, Iterator2 to, size_t to_size) {
            const size_t min_size = std::min(from_size, to_size);
            std::copy(from, from + min_size, to);
            if (from_size > to_size) {
                std::uninitialized_copy(from + min_size, from + from_size, to + min_size);
            } else {
        ...
        template <class Function> void populate_mem_forward(value_type *mem, std::size_t n, Function const &op) {
            std::size_t idx = 0;
            {
                auto rollback = make_guard([&] {
                    for (std::size_t i = 0; i < idx; ++i) {
        ...
        /// Unconst an iterator
        static iterator unconst(const_iterator it) { return iterator(const_cast<value_type *>(it.base())); }

      private:
        /// \brief A pointer to the heap and the capacity of the array in the heap
    ...
    /// Type deduction
    template <class T, class... U> vector(T, U...) -> vector<T, 1 + sizeof...(U)>;

    /// \brief operator== for small arrays
    template <class T, size_t N, class A, class H, class S>
    constexpr bool operator==(const vector<T, N, A, H, S> &x, const vector<T, N, A, H, S> &y) {
    ...
    /// \brief swap the contents of two small arrays
    template <class T, size_t N, class A, class H, class S>
    void swap(vector<T, N, A, H, S> &x, vector<T, N, A, H, S> &y) noexcept(noexcept(x.swap(y))) {
        x.swap(y);
    }
